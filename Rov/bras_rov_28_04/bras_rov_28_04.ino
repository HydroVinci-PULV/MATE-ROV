...
const int Pin7 = A7;
const int pwmMin = 120;//120;
const int pwmMax = 630; //630;
const int analogMin = 0;
const int analogMax = 1023; 
const int medianPWM = 614;
...
void resetPositionServo(int pinServo, int medianVal){

  pwm.setPWM(pinServo, 0, medianVal);
...
/*
  resetPositionServo(8, medianPWM);
  resetPositionServo(9, medianPWM);
  resetPositionServo(10, medianPWM);
  resetPositionServo(11, medianPWM);
  resetPositionServo(12, medianPWM);
*/
  ...
  //Serial.print("P0: ");
  int val0Calibrated = constrain(val0, 0, 400);
  int pwmval0 = map((val0Calibrated + previousVal0) / 2, analogMin, analogMax, pwmMin, pwmMax);  
  //Serial.print(pwmval0);
  ...
  //Serial.print("P2: ");
  int pwmval2 = map(-val2, -analogMax, -analogMin, pwmMin, pwmMax);  
  ...
  //Serial.print("P4: ");
  int pwmval4 = map(val4, analogMin, analogMax, pwmMin, pwmMax);
  //Serial.print(pwmval4);
  int val4Calibrated = constrain(pwmval4, 250, 550);
  pwm.setPWM(12, 0, val4Calibrated);
  delay(5);
  previousVal4 = val4;

  printDebug(pwmval4, val4Calibrated);
  Serial.println();
